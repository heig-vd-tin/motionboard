...
void EPwm1_InitGpio(void)
{
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO0 =
        EN_PULLUP;  // Enable pull-up on GPIO0 (EPWM1A)
    GpioCtrlRegs.GPAPUD.bit.GPIO1 =
        EN_PULLUP;  // Enable pull-up on GPIO1 (EPWM1B)
    GpioCtrlRegs.GPAMUX1.bit.GPIO0 =
        MUX_GPIO0_EPWM1A;  // Configure GPIO0 as EPWM1A
    GpioCtrlRegs.GPAMUX1.bit.GPIO1 =
        MUX_GPIO1_EPWM1B;  // Configure GPIO1 as EPWM1B
    EDIS;
}
...
void EPwm2_InitGpio(void)
{
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO2 =
        EN_PULLUP;  // Enable pull-up on GPIO2 (EPWM2A)
    GpioCtrlRegs.GPAPUD.bit.GPIO3 =
        EN_PULLUP;  // Enable pull-up on GPIO3 (EPW2B)
    GpioCtrlRegs.GPAMUX1.bit.GPIO2 =
        MUX_GPIO2_EPWM2A;  // Configure GPIO2 as EPWM2A
    GpioCtrlRegs.GPAMUX1.bit.GPIO3 =
        MUX_GPIO3_EPWM2B;  // Configure GPIO3 as EPWM2B
    EDIS;
}

void EPwm3_InitGpio(void)
{
    EALLOW;
    GpioCtrlRegs.GPAPUD.bit.GPIO4 =
        EN_PULLUP;  // Enable pull-up on GPIO4 (EPWM3A)
    GpioCtrlRegs.GPAPUD.bit.GPIO5 =
        EN_PULLUP;  // Enable pull-up on GPIO5 (EPWM3B)
    GpioCtrlRegs.GPAMUX1.bit.GPIO4 =
        MUX_GPIO4_EPWM3A;  // Configure GPIO4 as EPWM3A
    GpioCtrlRegs.GPAMUX1.bit.GPIO5 =
        MUX_GPIO5_EPWM3B;  // Configure GPIO5 as EPWM3B
    EDIS;
}

void EPwm4_InitGpio(void)
{
    EALLOW;
    GpioCtrlRegs.GPAPUD.bit.GPIO6 =
        EN_PULLUP;  // Enable pull-up on GPIO6 (EPWM4A)
    GpioCtrlRegs.GPAPUD.bit.GPIO7 =
        EN_PULLUP;  // Enable pull-up on GPIO7 (EPWM4B)
    GpioCtrlRegs.GPAMUX1.bit.GPIO6 =
        MUX_GPIO6_EPWM4A;  // Configure GPIO6 as EPWM4A
    GpioCtrlRegs.GPAMUX1.bit.GPIO7 =
        MUX_GPIO7_EPWM4B;  // Configure GPIO7 as EPWM4B
    EDIS;
}

void EPwm5_InitGpio(void)
{
    EALLOW;
    GpioCtrlRegs.GPAPUD.bit.GPIO8 =
        EN_PULLUP;  // Enable pull-up on GPIO8 (EPWM5A)
    GpioCtrlRegs.GPAPUD.bit.GPIO9 =
        EN_PULLUP;  // Enable pull-up on GPIO9 (EPWM5B)
    GpioCtrlRegs.GPAMUX1.bit.GPIO8 =
        MUX_GPIO8_EPWM5A;  // Configure GPIO8 as EPWM5A
    GpioCtrlRegs.GPAMUX1.bit.GPIO9 =
        MUX_GPIO9_EPWM5B;  // Configure GPIO9 as EPWM5B
    EDIS;
}

void EPwm6_InitGpio(void)
{
    EALLOW;
    GpioCtrlRegs.GPAPUD.bit.GPIO10 =
        EN_PULLUP;  // Enable pull-up on GPIO10 (EPWM10A)
    GpioCtrlRegs.GPAPUD.bit.GPIO11 =
        EN_PULLUP;  // Enable pull-up on GPIO11 (EPWM11B)
    GpioCtrlRegs.GPAMUX1.bit.GPIO10 =
        MUX_GPIO10_EPWM6A;  // Configure GPIO10 as EPWM6A
    GpioCtrlRegs.GPAMUX1.bit.GPIO11 =
        MUX_GPIO11_EPWM6B;  // Configure GPIO11 as EPWM6B
    EDIS;
}
...
{
    EALLOW;
    GpioCtrlRegs.GPAPUD.bit.GPIO12 =
        EN_PULLUP;  // Enable pull-up on GPIO12 (TZ1)
    GpioCtrlRegs.GPAQSEL1.bit.GPIO12 =
        GPXQSELX_ASYNC;  // Asynch input GPIO12 (TZ1)
    GpioCtrlRegs.GPAMUX1.bit.GPIO12 =
        MUX_GPIO12_TZ1;  // Configure GPIO12 as TZ1
    EDIS;
}
...
void EPwm1_ConfigureRegisters()
{
    ...
    // Action-Qualifier Submodule Registers
    {
        ...
        EPwm1Regs.AQCTLA.bit.CAU =
            1;  // Output high when the counter is incrementing
        EPwm1Regs.AQCTLA.bit.CAD =
            2;  // Output low when the counter is decrementing
    }
    ...
    // Trip-Zone Submodule Control and Status Registers
    {
        ...
        // EPwm1Regs.TZSEL.bit.OSHT1 = 1;     // Enable TZ1 as a one-shot trip
        // source

        // Trip-Zone Control Register
        EPwm1Regs.TZCTL.bit.TZA =
            1;  // Force EPWMxA to a low state if trip event occurs
        EPwm1Regs.TZCTL.bit.TZB =
            1;  // Force EPWMxB to a low state if trip event occurs
    ...
    // Time-Base Configuration
    {
        ...
        EPwm1Regs.TBCTL.bit.PRDLD =
            1;  // Directly accesses to the TBPRD active Register
    ...
    // Event-Trigger Submodule Registers
    {
        ...
        EPwm1Regs.ETPS.bit.SOCAPRD =
            1;  // Generate SOCA event on the first event ETPS
        ...
        EPwm1Regs.ETSEL.bit.SOCASEL =
            7;  // Enable event: time-base counter equal to CMPB when the timer
                // isdecrementing
        EPwm1Regs.ETSEL.bit.SOCAEN = 1;  // Enable EPWMxSOCA pulse

        EPwm1Regs.ETSEL.bit.INTSEL =
            1;  // Enable event time-base counter equal to zero (TBCTR=0x0000)
...
void EPwmSlave_ConfigureRegisters(volatile struct EPWM_REGS* EPwmSlaveRegs)
{
    ...
    EPwmSlaveRegs->AQCTLA.bit.CAU =
        1;  // Output high when the counter is incrementing
    EPwmSlaveRegs->AQCTLA.bit.CAD =
        2;  // Output low when the counter is decrementing
    ...
    EALLOW;
    // EPwmSlaveRegs->TZSEL.bit.OSHT1 = 1;      // Enable TZ1 as a one-shot trip
    // source
    EPwmSlaveRegs->TZCTL.bit.TZA =
        1;  // Force EPWMxA to a low state if trip event occurs
    EPwmSlaveRegs->TZCTL.bit.TZB =
        1;  // Force EPWMxB to a low state if trip event occurs
    EDIS;

    EPwmSlaveRegs->TZEINT.bit.OST = 0;  // Disable one-shot interrupt generation
    EPwmSlaveRegs->TZEINT.bit.CBC =
        0;  // Disable cycle-by-cycle interrupt generation
    ...
    EPwmSlaveRegs->TBCTL.bit.PHSDIR =
        1;  // Count down after the synchronization event
    EPwmSlaveRegs->TBCTL.bit.SYNCOSEL = 0;  // Sync flow-through
    EPwmSlaveRegs->TBCTL.bit.PRDLD =
        1;  // Directly accesses to the TBPRD active Register
...
void InitEPwm(void)
{
    EPwm_InitRegisters();  // Very Important Remark: This Initialisation must be
                           // before GPIO init.
    EPwm_InitGpio();
...
void EPwm_MotorA(three_phases u)
{
    ...
    EPwm1Regs.CMPA.half.CMPA =
        _IQmpy((_iq)u.w + 0x8000, (_iq)(HALF_SP - 2 * SAT_SP)) + SAT_SP;
    EPwm2Regs.CMPA.half.CMPA =
        _IQmpy((_iq)u.v + 0x8000, (_iq)(HALF_SP - 2 * SAT_SP)) + SAT_SP;
    EPwm3Regs.CMPA.half.CMPA =
        _IQmpy((_iq)u.u + 0x8000, (_iq)(HALF_SP - 2 * SAT_SP)) + SAT_SP;
}
