...
interrupt void pwmSyncIsr(void)
{
  ...
  if(xyCount++ > XY_INTERVAL)
  {
    ...
    if (++syncCount >= SYNC_INTERVAL) {
        ...
        if (global.mva.sync == 1) {
            global.mva.sync = 0;
            bangbang_mvt(&bangbangA, global.bba.acceleration,
                         global.bba.deceleration, global.bba.velocity,
                         global.bba.position);
        }
        if (global.mvb.sync == 1) {
            global.mvb.sync = 0;
            bangbang_mvt(&bangbangB, global.bbb.acceleration,
                         global.bbb.deceleration, global.bbb.velocity,
                         global.bbb.position);
        }
        ...
        if (global.mca.ctrl_type == BANGBANG &&
            global.mcb.ctrl_type == BANGBANG) {
        ...
        if (global.mca.ctrl_type >= POSITION)
            global.mva.velocity =
                RegulPi_p(motorA.position, global.mva.position, &bldc_a);
        if (global.mca.ctrl_type >= VELOCITY)
            global.mva.torque =
                RegulPi_v(motorA.velocity, global.mva.velocity, &bldc_a);

        if (global.mcb.ctrl_type >= POSITION)
            global.mvb.velocity =
                RegulPi_p(motorB.position, global.mvb.position, &bldc_b);
        if (global.mcb.ctrl_type >= VELOCITY)
            global.mvb.torque =
                RegulPi_v(motorB.velocity, global.mvb.velocity, &bldc_b);
    }
    ...
    if (global.mca.ctrl_type >= TORQUE) {
        ...
        moche = 1;
        utria = TorqueController(adcResults.motorA, motorA.theta_sincos, torque,
                                 &bldc_a);
    ...
    if (global.mcb.ctrl_type >= TORQUE) {
        ...
        moche = 0;
        utrib = TorqueController(adcResults.motorB, motorB.theta_sincos, torque,
                                 &bldc_b);
    ...
    // Revolution blink
    if (oldrevA != motorA.revolution || oldrevB != motorB.revolution) {
...
three_phases TorqueController(three_phases currents, tri_theta angle,
                              int torque, bldc_controller *bldc)
{
...
void UpdateConfiguration()
{
    ...
    Init_bangbang(&bangbangA, 4000, global.mca.encoder_resolution,
                  global.mca.bang_acc_max, global.mca.bang_dec_max,
                  global.mca.bang_spd_max);
    Init_bangbang(&bangbangB, 4000, global.mcb.encoder_resolution,
                  global.mcb.bang_acc_max, global.mcb.bang_dec_max,
...
void main(void)
{
    ...
    PieCtrlRegs.PIEIER3.bit.INTx1 =
        1;          // Set active EPWM1_INT interrupt (INT3.x)
    IER |= M_INT1;  // Set active interrupt level INT1.y
    PieCtrlRegs.PIEIER1.bit.INTx6 =
        1;  // Set active SEQ1_INT  interrupt (INT1.x)
