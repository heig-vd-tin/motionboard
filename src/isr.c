 ...
 * @file    isr.c
 * @brief   Interrupt service routines module
 * @author  Yves Chevallier <yves.chevallier@kalios.ch>
 * @svn     $Id: isr.c 531 2008-12-14 20:35:55Z ychevall@heig-vd.ch $
 */
...
/// PI Controller
////////////////////////////////////////////////////////////////////////////////
float pi_controller_dz (float w, float y, float kp, float gi, float *integral, float window)
{
	float e; 
	e = w-y; 
...
/// PID Controller
//////////////////////////////////////////////////////////////////////////////// 
float pid_controller_dz (float w, float y, float kp, float gi, float td, float *ui, float window, float *e_old)
{
	float e, u, ud; 
	e = w-y; 
...
float HallGetSpeed()
{
	...
	if(hallOldPosition != hallPosition)
	{
		...
		if(countStep < 100)
		{
			...
			if(hallPosition == 0)
				velocity = (60*conf.env.pwm_frequency)/(conf.mca.poles_numbers*countRevolution); // rpm
		}
		else
		{
			// Number of count 0 - 1 (Low and Medium Speed)
			velocity = (60*conf.env.pwm_frequency)/(6.0*conf.mca.poles_numbers*countStep); // rpm
...
/*		break;
	...
	{
		u1.d = pi_controller(conf.mca.desat, y1.d, conf.mca.tq_kp, conf.mca.tq_gi, &integral2);
		conf.mva.pwm = pi_controller(conf.mva.torque, y1.q, conf.mca.tq_kp, conf.mca.tq_gi, &integral1); 
	}
	else
		u1.d = 0.0; 
	...
	{
		u2.d = pi_controller(conf.mcb.desat , y2.d, conf.mcb.tq_kp, conf.mcb.tq_gi, &integral4);
		conf.mvb.pwm = pi_controller(conf.mvb.torque, y2.q, conf.mcb.tq_kp, conf.mcb.tq_gi, &integral3); 
	}
	else
		u2.d = 0.0; 
	...
	{
		inverse_park_clarke(u1.d, conf.mva.pwm, &pwm1.u, &pwm1.v, &pwm1.w, theta1);
	...
	{
		inverse_park_clarke(u2.d, conf.mvb.pwm, &pwm2.u, &pwm2.v, &pwm2.w, theta2);
...
////////////////////////////////////////////////////////////////////////////////
/// cpu_timer0 ISR (System interruption)
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
/// cpu_timer1 ISR (Velocity and position computing)
////////////////////////////////////////////////////////////////////////////////
...
{	
	...
	else
	{
		...
		{
			conf.mvb.torque = pid_controller_dz (conf.mva.position, l1, conf.mcb.sp_kp, conf.mcb.sp_gi, conf.mcb.sp_dz, &integral5, conf.mcb.pos_dz, &ud1);
		}
		if(conf.mca.ctrl_type >= POSITION)
		{
			conf.mva.torque = pid_controller_dz (conf.mvb.position, l2, conf.mca.sp_kp, conf.mca.sp_gi, conf.mca.sp_dz, &integral6, conf.mca.pos_dz, &ud2);
		}
	}
/*
	...
	// Odometry
	EQep_xy(&eqep1, &eqep2, (float*)&conf.odo.x, (float*)&conf.odo.y, (float*)&conf.odo.orientation); 
} 
