/**
 * Logiciel de contrôle de moteur pour la carte "motionboard"
 */
#include "bangbang.h"
...
void Init_bangbang(handle_bangbang *h, int fech, int coder, int accMax,
                   int decMax, long vMax)
{
 ...
 */
void bangbang_mvt(handle_bangbang *h, int acceleration, int deceleration,
                  int nom_speed, int64 position)
{
...
{
    if (h->busy_flag == 0) return position;
    if (h->start_flag == 1) {
        h->start_flag = 0;
        h->pos_init = position;
    }
 ...
 * @param handle Handle of this bangbang calculation
 */
void bangbang_smooth_stop(handle_bangbang *h) { h->stop_flag = 1; }
