...
void InitAdc(void)
{
    ...
    AdcRegs.ADCREFSEL.bit.REF_SEL =
        2;                             // External reference, 1.500V on ADCREFIN
    AdcRegs.ADCTRL1.bit.ACQ_PS = 1;    // Acquisition window size
    AdcRegs.ADCTRL1.bit.SEQ_CASC = 1;  // Cascaded mode

    AdcRegs.ADCTRL3.bit.ADCBGRFDN =
        3;  // Bandgap and reference circuitry is powered up
    AdcRegs.ADCTRL3.bit.ADCPWDN =
        1;  // Analog circuitry inside the core is powered up
    AdcRegs.ADCTRL3.bit.ADCCLKPS =
        1;  // ADCLK = HISPCLK(SYSCLK/2)/ADCCLKPS/2 = 50e6/4 = 12.5MHz
