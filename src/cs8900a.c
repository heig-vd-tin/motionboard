...
///
/// cs8900a ethernet controller driver: The CS8900A is connected in IO-mode (address lines a0,a1,a2 of the
/// dsp are connected to the sa1,sa2,sa3 of the CS8900A, sa8 and sa6 are connected to hi, all other address lines 
/// are tied to lo). The base address is set to xintf zone 2 (0x080000), so the chip select signal xzcs2 is used
/// for selecting the CS8900A. For interrupt requests the XINT13 is used, because this interrupt has got lowest
/// priority.
///
/// receiver: The received data is not read in io/mode, it is read by using the packet page pointer and
/// the packet page data. The advantage by using this mode, you need not to read the complete frame before
...
#define SET_PPP(ADDR) CS8900A_PPP=ADDR; asm(" rpt #(3-1) || nop") /* wait for correct timing */
...
#define READ(ADDR,VALUE) SET_PPP(ADDR); VALUE=PPD
/** writes to cs8900a memory
  * @param ADDR address to read from
  * @param VALUE value to write */
#define WRITE(ADDR,VALUE) CS8900A_PPP=ADDR; PPD=VALUE
...
{ 
  ...
  /* initialize bus timing */
	...
	XintfRegs.XTIMING6.bit.READYMODE = 0; // Does not matter because XREADY is ignored 
...
//  XINTFREGS_XTIMING=0x00431CB5;    /* bit 31,30,29,28,27,26,25,24=reserved
              ...
              //                      * bit 15=0: READYMODE does not matter because XREADY is ignored
            //                        * bit 14=0: USERREADY=0 to ignore XREADY
          //                          * bit 13,12=01: XRDLEAD=1 (4 Cycles)
        //                            * bit 11,10,9=110: XRDACTIVE=6 (24 cycles=160ns)
      //                              * bit 8,7=01: XRDTAIL=1 (4 cycles=26ns)
    //                                * bit 6,5=01: XWRLEAD=1 (4 cycles=26ns)
  //                                  * bit 4,3,2=101: XWRACTIVE=5 (20 cycles=133ns)
//                                    			* bit 1,0=01: XWRTAIL=1 (4 cycles=26ns) */

                        /* initialize external interrupt */
  //XIntruptRegs.XNMICR.all=0x0006;  /* bit 15,14,13,12,11,10,9,8,7,6,5,4,3=reserved
  ...
  do { READ(REG16_SELF_STATUS,H); } while((H&0x0080)==0); /* wait for end of reset */
  
  /* setup cs8900a */
  WRITE(INTERRUPT_NUMBER,0x0000);      /* set interrupt number */
  WRITE(REG3_RECEIVER_CONFIG,0x0100);  /* interrupt when frame is received without errors */
  WRITE(REG5_RECEIVER_CONTROL,0x0D00); /* accept frames without errors and with broadcast or individual address */
...
{ 
  ...
  CS8900A_TXCMD=0x00C0;    /* start transmission after complete frame is written to cs8900a */
