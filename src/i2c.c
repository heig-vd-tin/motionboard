 ...
 */
char read_memory(int reg, int byte)
{
    static long long buffer64 = 0;
    switch (reg) {
        case SCMB_REG_BB_BUSY_A:
            return global.bba.busy;
        case SCMB_REG_BB_BUSY_B:
            return global.bbb.busy;
        case SCMB_REG_BB_BUSY:
            return (global.bbb.busy || global.bba.busy);

        case SCMB_REG_XYP_READ_X:
            return (char)(global.xyp.x >> (8 * byte));
        case SCMB_REG_XYP_READ_Y:
            return (char)(global.xyp.y >> (8 * byte));
 ...
 * Write memory
 */
void write_memory(char* data, int length)
{
    switch (data[0])  // Data index 0 is address pointer
    {
        ...
        case SCMB_REG_MODE_A:
            if (data[1] < 6) global.mca.ctrl_type = data[1];
            break;
        case SCMB_REG_MODE_B:
            if (data[1] < 6) global.mcb.ctrl_type = data[1];
            break;
        ...
        case SCMB_REG_SYNC:
            if (data[1] == 0x5F) global.env.sync = 1;
            break;
        case SCMB_REG_SYNC_A:
            if (data[1] == 0x5F) global.mva.sync = 1;
            break;
        case SCMB_REG_SYNC_B:
            if (data[1] == 0x5F) global.mvb.sync = 1;
            break;
        ...
        case SCMB_REG_TQ_KP_A:
            global.mca.tq_kp = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_TQ_GI_A:
            global.mca.tq_gi = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_SP_KP_A:
            global.mca.sp_kp = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_SP_GI_A:
            global.mca.sp_gi = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_POS_KP_A:
            global.mca.pos_kp = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_POS_GI_A:
            global.mca.pos_gi = (int)(data[2] << 8) + (int)data[1];
            break;

        // Adaptation des gains des régulateurs moteur B
        case SCMB_REG_TQ_KP_B:
            global.mcb.tq_kp = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_TQ_GI_B:
            global.mcb.tq_gi = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_SP_KP_B:
            global.mcb.sp_kp = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_SP_GI_B:
            global.mcb.sp_gi = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_POS_KP_B:
            global.mcb.pos_kp = (int)(data[2] << 8) + (int)data[1];
            break;
        case SCMB_REG_POS_GI_B:
            global.mcb.pos_gi = (int)(data[2] << 8) + (int)data[1];
            break;
 ...
 * de stackoverflow. Aussi, le mode polling est préférable dans cette
 * application A titre d'information cette function reste présente dans ce
 * fichier
 */
